...
/**
 * PrepayShopOrderSubmitNode
 *
 * @author ziyuan
 * @since 2017-03-29
 */
public final class PrepayShopOrderSubmitNode extends BaseSubmitOrderNode {

    public static final PrepayShopOrderSubmitNode INSTANCE = new PrepayShopOrderSubmitNode();

    public static PrepayShopOrderSubmitNode getINSTANCE() {
        return INSTANCE;
    }
    ...
    public Result<String> submitOrder(RequestContext requestContext, Response response) {
        ...
        if (isAddDish || !isPrepay) {
            System.out.println("prepay shop order error, isAddDish:" + isAddDish + ",isPrepay:" + isPrepay + ",baseParam.orderId:" + param.orderId + ",orderId is Blank:" + StringUtils.isBlank(param.orderId));
